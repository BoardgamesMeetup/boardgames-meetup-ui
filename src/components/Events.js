...
const Events = () => {
    ...
    const restoreStateFromLocalStorage = () => {
      try {
        ...
        if (savedFilters) {
          ...
          restoredFilters.fromDate = savedFilters.fromDate || null;
          restoredFilters.toDate = savedFilters.toDate || null;
  ...
  const validateDates = (fromDate, toDate) => {
    ...
    if (fromDate !== null && fromDate !== '') {
      ...
      if (fromDateObj < minDate) {
        newErrors.fromDate = 'Starting Date must be from 2025 onwards';
      }
      if (fromDateObj > maxDate) {
        newErrors.fromDate = 'Starting Date cannot be past 2025';
      }
      if (toDate !== null && toDate !== '') {
        const toDateObj = new Date(toDate);
        if (fromDateObj > toDateObj) {
          newErrors.fromDate = 'Starting Date must be before Ending date';
        }
      }
    }
    ...
    if (toDate !== null && toDate !== '') {
      ...
      if (toDateObj < minDate) {
        newErrors.toDate = 'Ending date must be from 2025 onwards';
      }
      if (toDateObj > maxDate) {
        newErrors.toDate = 'Ending date cannot be past 2025';
      }
      if (fromDate !== null && fromDate !== '') {
        const fromDateObj = new Date(fromDate);
        if (toDateObj < fromDateObj) {
          newErrors.toDate = 'Ending date must be after starting date';
        }
      }
    }
  ...
  const createSearchParams = (searchFilters) => {
    ...
    if (searchFilters.toDate !== null && searchFilters.toDate !== '') {
      params.toDate = searchFilters.toDate.trim();
    }
    if (searchFilters.boardgame
      && searchFilters.boardgame
        .trim()) {
      params.boardgame
        = searchFilters.boardgame
          .trim();
